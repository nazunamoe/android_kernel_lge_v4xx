...
#ifdef CONFIG_LGE_PM_NEED_TO_MONITORING_QCT_PATCH //                              
...
qpnp_chg_regulator_batfet_set(struct qpnp_chg_chip *chip, bool enable)
{
	...
	if (chip->charging_disabled || !chip->bat_if_base)
		return rc;
#if defined(CONFIG_LGE_PM_DISABLE_ULPM_MODE) || defined(CONFIG_MACH_MSM8926_B2LN_KR) || defined(CONFIG_MACH_MSM8926_JAGN_KR)
	if (chip->type == SMBB)
		rc = qpnp_chg_masked_write(chip,
...
static void devices_state_monitor_work (struct work_struct *work) {
	...
	usb_in_volt_avg =  get_vusb_averaged(chip, counter) / 1000;			//                   
	if (usb_in) {
		if (chip->chg_done)
