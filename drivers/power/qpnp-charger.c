...
#ifdef CONFIG_MACH_MSM8226_E8LTE_KR
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (!(chip->usb_present ^ usb_present)) {
		usb_present = qpnp_chg_is_usb_chg_plugged_in(chip);
		pr_debug("need to check usbin-valid triggered: %d \n", usb_present);
	}
#endif
...
#ifdef CONFIG_LGE_PM_CHARGING_TEMP_SCENARIO
	schedule_delayed_work(&chip->battemp_work, MONITOR_BATTEMP_POLLING_PERIOD / 12);
#endif
#ifdef CONFIG_LGE_PM
	chip->chg_fail_irq_happen = false;
	qpnp_chg = chip;
